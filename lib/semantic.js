...
export function resolveRevisionHistory(baseMode, previousCandidate, previousInstructions = []) {
    if (baseMode === 'original') return { previousCandidate: '', previousInstructions: [] };
    return {
        previousCandidate: String(previousCandidate ?? ''),
        previousInstructions: Array.isArray(previousInstructions) ? previousInstructions.slice() : [],
    };
}
