...
export function validateImpactPlan(plan, paragraphs, focusIds, referenceIds = []) {
    ...
    const modelFocus = focusIds.length
        ? focusIds.map(paragraphId => ({ paragraphId, selectedQuote: knownParagraphs.get(paragraphId)?.text ?? '', reason: '用户直接指定的强修改区' }))
        : Array.from(plan.focusRegions ?? [])
            .filter(region => validRegion(region, knownParagraphs, 'selectedQuote'))
            .map(region => ({
                paragraphId: region.paragraphId,
                selectedQuote: region.selectedQuote,
...
export function buildImpactPrompt(task) {
    ...
    return [
        ...
        task.focusIds?.length
            ? 'focusParagraphIds 是用户直接指定且不可取消的强修改区。linkedRegions 只能包含人物、指代、因果、伏笔或结果直接相关的段落。transitionRegions 只用于必要衔接。'
