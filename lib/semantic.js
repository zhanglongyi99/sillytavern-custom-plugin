    ...
    for (let anchorIndex = 0; anchorIndex < anchors.length - 1; anchorIndex++) {
        ...
        for (let index = 0; index < size; index++) {
            ...
            changes.push({
                ...
                originalIndex: originalParagraph ? leftStart + 1 + index : null,
                candidateIndex: candidateParagraph ? rightStart + 1 + index : null,
...
export function composeRevisionFromDecisions(originalMessage, revisedMessage, acceptedChangeIds) {
    ...
    for (let anchorIndex = 0; anchorIndex < anchors.length - 1; anchorIndex++) {
        ...
        for (let index = 0; index < size; index++) {
            ...
            const chosen = accepted.has(changeId) ? rightGap[index] : leftGap[index];
            if (chosen?.text) blocks.push(chosen.text);
        }
        if (leftEnd < original.length) blocks.push(original[leftEnd].text);
    }
